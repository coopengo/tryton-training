...
class FuseBooks(Wizard):
    ...
    def transition_check_compatibility(self):
        ...
        bad_matches = [k for k, v in values.iteritems() if not v[1]]
        if bad_matches:
    ...
    def default_preview(self, name):
        book = {k: v for k, (v, _) in self._get_merge_values().iteritems()}
    ...
    def transition_merge(self):
        ...
        for fname, (value, _) in self._get_merge_values().iteritems():
